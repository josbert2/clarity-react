  ...
  return (
    <TabProvider value={{ value }}>
      <TabsPrimitive.Trigger
        ref={ref}
        className={cn(
          "group h-full inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-white shadow-ln-button-white text-[#3d3d3d]",
          className
        )}
        value={value}
        {...props}
    ...
    <AnimatePresence initial={false}>
      {isActive && (
        <motion.div
          ...
          {...props}
        >
          <span className="ml-1 text-[#3d3d3d]">{children}</span>
        </motion.div>
      )}
    </AnimatePresence>
  )
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Content
    ref={ref}
    className={cn(
      "mt-2 ring-offset-background bg-background shadow-ln-xs rounded-[11px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
      className
    )}
    {...props}
  />
